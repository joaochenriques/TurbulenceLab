...
class ChData:
    def __init__( self, DevChName, SignalName, Units, V_min, V_max, Coeffs, Clip ):
...
DAQ_Prefix    = 'Turbulence_Lab_G01'   # data file per group 
DAQ_Test_Name = 'Test_04'              # test name
...
DAQ_Chs = [ ChData( DevChName='Dev1/ai0', SignalName='hotwire', Units='[m/s]',
                    V_min=0.0,  V_max=1.0, Coeffs=CalibPoly, Clip=(0.0,45.0) ), ]
...
def animate( i, xs, ys ):
    for Ch in range( NumChs ):
        Buffer = np.clip( np.polyval( DAQ_Chs[Ch].Coeffs, ys[Ch] ), DAQ_Chs[Ch].Clip[0], DAQ_Chs[Ch].Clip[1] )
